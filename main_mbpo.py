...
def readParser():
    ...
    parser.add_argument('--env_name', default="halfcheetah-medium-v0",
                        help='D4RL environment (default: halfcheetah-medium-v0)')
    ...
    parser.add_argument('--model_train_batch_size', type=int, default=100000, metavar='A',
                        help='batch size for training policy')
    ...
    parser.add_argument('--test_interval', default=1000, metavar='A',
                        help='test running interval (gradient steps)')
    parser.add_argument('--cuda', default=True, action="store_true",
                        help='run on CUDA (default: True)')
    return parser.parse_args()


def train(args, env_sampler, predict_env, agent, env_pool, model_pool,logfile):
    total_step = 0
    reward_sum = 0
    rollout_length = 1
    gradient_step=0
    ...
    for epoch_step in range(args.num_epoch):
        ...
        for i in count():
            ...
            if cur_step > 0 and cur_step % args.model_train_freq == 0 and args.real_ratio < 1.0:
                ...
                print('model training takes:',str(int(time.time()-st)),'seconds')
                new_rollout_length = set_rollout_length(args, epoch_step)
                #print(new_rollout_length)
                if rollout_length != new_rollout_length:
                    rollout_length = new_rollout_length
                    model_pool = resize_model_pool(args, rollout_length, model_pool)
                st=time.time()
                print('rollouts generation')
                rollout_model(args, predict_env, agent, model_pool, env_pool, rollout_length)
                print('generating rollouts takes',int(time.time()-st),'seconds')
            if len(model_pool)>args.min_pool_size:
                #print('policy training training')
                #st=time.time()
                steps,gradient_step,test_flag=train_policy_repeats(args, total_step, train_policy_steps, cur_step, env_pool, model_pool, agent,gradient_step,test_flag)
                train_policy_steps += steps
                #print('policy training takes',int(time.time()-st),'seconds')
            total_step += 1
            #print(total_step)
            if gradient_step % args.test_interval == 0 and not test_flag:
                test_flag=True
                print('Evaluating')
                st=time.time()
                rewards=[]
                ...
                logfile.write(str(np.mean(rewards))+'\n')
                logfile.flush()
                ...
                print('Gradient steps:',gradient_step)
                print('Step:',total_step,'reward mean:', np.mean(rewards),'evaluation takes',int(time.time()-st))

def fill_static_env_pool(args,env_pool,dataset):
    obs=dataset['observations']
    obs_=dataset['next_observations']
    act=dataset['actions']
    reward=dataset['rewards']
    done=[dataset['terminals'][i] for i in range(len(dataset['terminals']))]
    for i in range(len(dataset['terminals'])):
        env_pool.push(obs[i],act[i],reward[i],obs_[i],done[i])
    return env_pool
...
def train_predict_model(args, env_pool, predict_env):
    ...
    state, action, reward, next_state, done = env_pool.sample(256)
...
def train_policy_repeats(args, total_step, train_step, cur_step, env_pool, model_pool, agent,gradient_step,test_flag):
    if total_step % args.train_every_n_steps > 0:
        return 0,gradient_step,test_flag
    ...
    return args.num_train_repeat,gradient_step,False
...
def main(args=None):
    ...
    # Initial pool and dataset for env
    dataset=d4rl.qlearning_dataset(env)
    env_pool = ReplayMemory(args.replay_size)
    env_pool=fill_static_env_pool(args,env_pool,dataset)
    ...
    logfile=open('./results/'+args.env_name+'/'+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())+'.txt','w+')
    print(args)
    print('Start training')
    print(len(env_pool))
    train(args, env_sampler, predict_env, agent, env_pool, model_pool,logfile)
    logfile.close()
